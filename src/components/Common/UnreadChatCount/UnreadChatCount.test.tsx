...
describe('UnreadChatCount 컴포넌트', () => {
  it('숫자가 올바르게 렌더링됩니다', () => {
    const { getByText } = renderWithTheme(<UnreadChatCount unreadCount={5} />);

    expect(getByText('5')).toBeTruthy();
  });

  it('큰 숫자(99)가 올바르게 렌더링됩니다', () => {
    const { getByText } = renderWithTheme(<UnreadChatCount unreadCount={99} />);

    expect(getByText('99')).toBeTruthy();
  });

  it('0이 올바르게 렌더링됩니다', () => {
    const { getByText } = renderWithTheme(<UnreadChatCount unreadCount={0} />);
